...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['new-password']) && !empty($_POST['new-password'])) {
            ...
            $stmt = $connexion->prepare("UPDATE Professeur SET mdp_prof = :password WHERE Id_Professeur = :id");

            if ($stmt->execute(['password' => $newPassword, 'id' => $id_prof])) {
                // Redirect the user back to the profile page with a success message
                header('Location: ProfilP.php?success=1');
                exit();
            } else {
